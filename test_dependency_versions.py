...
def extract_version_from_pycff_file(filepath: Path) -> Optional[str]:
    with open(filepath, "r") as f:
        for line in f:
            match = re.search(
                r'^_*version[_,\s]*[:,=]\s*"?([0-9]+(?:\.[^\s"\.\,]+){2})"?', line
            )
            if match:
...
def extract_version_from_oml_file(filepath: Path, pattern: str) -> Optional[str]:
    with open(filepath, "r") as f:
        for line in f:
            match = re.search(
                pattern
...
# emu_sv -> emu_base
sv_dep = extract_version_from_oml_file(file_dir / "ci/emu_sv/pyproject.toml", "emu-base")
print(f" - emu-sv depends on emu-base version {sv_dep}")

# emu_mps -> emu_base
mps_dep = extract_version_from_oml_file(
    file_dir / "ci/emu_mps/pyproject.toml", "emu-base"
)
print(f" - emu-mps depends on emu-base version {mps_dep}")

# emu_base version
base_version = extract_version_from_pycff_file(file_dir / "emu_base/__init__.py")
print(f" - emu-base is version {base_version}")

# emu_sv version
sv_version = extract_version_from_pycff_file(file_dir / "emu_sv/__init__.py")
print(f" - emu-sv is version {sv_version}")

# emu_mps version
mps_version = extract_version_from_pycff_file(file_dir / "emu_mps/__init__.py")
print(f" - emu-mps is version {mps_version}")

# citation version

citation_version = extract_version_from_pycff_file(file_dir / "CITATION.cff")
...
pulser_pre_commit_version = extract_version_from_oml_file(
...
torch_pre_commit_version = extract_version_from_oml_file(
...
torch_pyproject_version = extract_version_from_oml_file(
    file_dir / "pyproject.toml", "torch"
)
...
torch_emu_base_version = extract_version_from_oml_file(
    file_dir / "ci/emu_base/pyproject.toml", "torch"
)
