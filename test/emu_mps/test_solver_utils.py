...
def test_right_baths_bell():
    # state = (|0> + |1>)^3 / norm
    mps_factor1 = torch.tensor([[[1, 0], [0, 1]]], dtype=dtype)
    mps_factor2 = torch.tensor(
        [[[1, 0], [0, 0]], [[0, 0], [0, 1]]], dtype=torch.complex128
    )
    mps_factor3 = torch.tensor([[[1], [0]], [[0], [1]]], dtype=dtype)

    # Hamiltonian X1*X2*X3
    mpo_factor = torch.tensor([[[[0], [1]], [[1], [0]]]], dtype=dtype)
    ...
    for i, b in enumerate(right_baths(state, obs, 1)):
        ...
        if i == 0:
            ...
            assert torch.allclose(
                b,
                torch.tensor([[[0, 1]], [[1, 0]]], dtype=dtype, device=b.device),
            )


def test_right_baths_total_magnetization():
    # Hamiltonian Z1+Z2+Z3
    mpo_factor1 = torch.tensor([[[[1, 1], [0, 0]], [[0, 0], [-1, 1]]]], dtype=dtype)
    ...
    # state |111>
    mps_factor = torch.tensor([[[0], [1]]], dtype=dtype)
    ...
    assert torch.allclose(
        baths[0], torch.ones(1, 1, 1, dtype=dtype, device=baths[0].device)
    )
    assert torch.allclose(
        baths[1],
        torch.tensor([[[1], [-1]]], dtype=dtype, device=baths[1].device),
    )
    assert torch.allclose(
        baths[2],
        torch.tensor([[[1], [-2]]], dtype=dtype, device=baths[2].device),
    )


def test_apply_2_site_effective_Hamiltonian():
    left_bath = torch.randn(4, 3, 4, dtype=dtype)
    right_bath = torch.randn(7, 6, 7, dtype=dtype)
    state = torch.randn(4, 4, 7, dtype=dtype)
    ...
    ham, right_bath = reshape_ham_rbath(ham, right_bath)
...
def test_krylov_exp_krylov_norm_tolerance():
    ...
    def op(x):
        return (
            torch.pi * 0.5j * apply_effective_Hamiltonian(x, ham, left_bath, right_bath)
        )
    ...
    assert torch.allclose(
        result,
        torch.tensor([[[0.0], [0.0], [1.0j], [0.0]]], dtype=dtype),
    )
...
def test_krylov_exp_krylov_exp_tolerance():
    ...
    state = torch.zeros(bath_size, 4, 1, dtype=dtype)
    state[0, 0, 0] = 1
    ...
    assert torch.allclose(result[:, 1:, 0], torch.zeros(bath_size, 3, dtype=dtype))
...
def test_evolve_single():
    left_bath = torch.rand(3, 4, 3, dtype=dtype)
    right_bath = torch.rand(4, 5, 4, dtype=dtype)
    state_factor = torch.rand(3, 2, 4, dtype=dtype)
    ham_factor = torch.rand(4, 2, 2, 5, dtype=dtype)
...
def test_evolve_pair():
    left_bath = torch.rand(3, 4, 3, dtype=dtype)
    right_bath = torch.rand(5, 6, 5, dtype=dtype)
    left_state_factor = torch.rand(3, 2, 4, dtype=dtype)
    right_state_factor = torch.rand(4, 2, 5, dtype=dtype)
    left_ham_factor = torch.rand(4, 2, 2, 5, dtype=dtype)
    right_ham_factor = torch.rand(5, 2, 2, 6, dtype=dtype)
