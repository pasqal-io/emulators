...
###
    L = input_mat.shape[0]
    rnd_permutations = (np.random.permutation(L) for _ in range(samples))

    #remove copy, it's not necessary
    opt_permutations = (
        minimize_bandwidth_impl(permute_matrix(input_mat.copy(), rnd_perm))
