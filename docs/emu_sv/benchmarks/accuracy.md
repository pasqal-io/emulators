...
For the quench sequence (where the Hamiltonian is constant in time), emu-sv shows better accuracy than Pulser. The evolution method used by emu-sv, evolving the system by assuming the Hamiltonian is constant for intervals of length `dt` is exact in this case. The `krylov_tolerance` configuration parameter is an estimate of the error incurred in each time-step, which experimentally overestimates the error for values < `1e-8`, which is the default value used in the above. As a consequence, the deviation from pulser shown in the graph is therefore dominated by the numerical error in pulser when doing the simulation. This is consistent with the accuracy settings used by pulser-simulation internally.
