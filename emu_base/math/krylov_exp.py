...
def krylov_exp_to_matrix(
    ...
    """
    Evolve a matrix M using the Krylov subspace method by calling krylov_exp in
    each column
    """
    # Initialize the result matrix
    result_matrix = torch.zeros_like(M)

    # Iterate over the columns of M
    for i in range(M.size(1)):
        v = M[:, i]

        # krylov_exp_impl function to each i-th column
        krylov_result = krylov_exp_impl(
            op,
            v,
            is_hermitian=is_hermitian,
        ...
        if not krylov_result.converged:
            raise RecursionError(
                "exponentiation algorithm did not converge to "
                "precision in allotted number of steps."
            )

        # Store the result in the i-th column of the result
        result_matrix[:, i] = krylov_result.result
